...
export default function TaskCard({ task, currentUserId }: TaskCardProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      className={`
        rounded-lg shadow-sm p-3 mb-2 cursor-pointer transition-all duration-200
        ${isOverdue 
          ? isDarkMode 
    ...
    >
      ...
      <div className="flex justify-between items-start mb-1.5">
        <span
          className={`
            px-2 py-0.5 rounded-full text-xs font-medium border transition-colors duration-300
            ${isDarkMode 
        ...
        {isOverdue && (
          <span className={`px-2 py-0.5 rounded-full text-xs font-medium border transition-colors duration-300 ${
            isDarkMode 
      ...
      {/* Task title */}
      <h3 className={`font-medium mb-1.5 overflow-hidden transition-colors duration-300 ${
      ...
      {/* Task description */}
      {task.description && (
        <p className={`text-sm mb-2 overflow-hidden transition-colors duration-300 ${
      ...
      {task.tags && task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {task.tags.slice(0, 3).map((tag, index) => (
            <span
              key={index}
              className={`px-1.5 py-0.5 text-xs rounded-md transition-colors duration-300 ${
                isDarkMode 
          ...
          {task.tags.length > 3 && (
            <span className={`px-1.5 py-0.5 text-xs rounded-md transition-colors duration-300 ${
              isDarkMode 
                ? 'bg-gray-700 text-gray-300' 
                : 'bg-gray-100 text-gray-700'
            }`}>
      ...
      <div className="flex justify-between items-center text-xs mt-2">
        <div className="flex items-center space-x-1.5">
          {task.dueDate && (
            <div className={`flex items-center space-x-0.5 px-1.5 py-0.5 rounded-md text-xs ${
              isOverdue 
                ? isDarkMode 
        ...
        <div className="flex items-center space-x-1">
          {task.assignedTo && (
            <div className="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-medium">
              {isAssignedToCurrentUser ? 'Me' : task.assignedTo.slice(0, 2).toUpperCase()}
            </div>
          )}
